...
generate_comprehensive_report() {
    ...
    INTEGRATED_REPORT="migration_verification.log"
...
cleanup_logs() {
    ...
    # 추가 정리 작업
# BROKEN_REF:     find . -name "*.log" -type f -mtime +$DAYS -delete 2>/dev/null || true
# BROKEN_REF:     find . -name "*_report_*.txt" -type f -mtime +$DAYS -delete 2>/dev/null || true
...
run_full_verification() {
    ...
    # 상태 보고 시작
    python3 -c "
from migration_status_reporter.py import docs/status.json
