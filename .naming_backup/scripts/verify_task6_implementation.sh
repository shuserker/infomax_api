...
if grep -A 50 "^start_watchhamster()" watchhamster_control_center.sh | grep -q ".naming_backup/config_data_backup/watchhamster.log"; then
...
# Requirement 2.2: WatchHamster v3.0 상태 확인 기능
print_section "📋 Requirement 2.2: WatchHamster v3.0 상태 확인 기능"
echo "WHEN 'WatchHamster v3.0 컴포넌트 정보가 표시 SHALL 된다"
echo
...
if grep -A 20 "^check_managed_processes()" watchhamster_control_center.sh | grep -q ".comprehensive_repair_backup/integrated_report_scheduler.py.backup_20250809_181656"; then
...
echo
echo "구현된 기능:"
echo "1. 🚀 WatchHamster v3.0 상태 확인"
