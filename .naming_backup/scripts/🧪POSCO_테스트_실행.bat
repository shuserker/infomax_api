...
if "%test_choice%"=="1" (
    ...
    if "%test_date%"=="" if "%test_time%"=="" (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type business
    ) else (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type business --test-date "%test_date%" --test-time "%test_time%"
    )
) else if "%test_choice%"=="2" (
    call :print_section "⏰ 지연 발행 알림 테스트"
    if "%test_date%"=="" if "%test_time%"=="" (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type delay
    ) else (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type delay --test-date "%test_date%" --test-time "%test_time%"
    )
) else if "%test_choice%"=="3" (
    call :print_section "📊 일일 통합 분석 리포트 테스트"
    if "%test_date%"=="" if "%test_time%"=="" (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type report
    ) else (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type report --test-date "%test_date%" --test-time "%test_time%"
    )
) else if "%test_choice%"=="4" (
    call :print_section "✅ 정시 발행 알림 테스트"
    if "%test_date%"=="" if "%test_time%"=="" (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type timely
    ) else (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type timely --test-date "%test_date%" --test-time "%test_time%"
    )
) else if "%test_choice%"=="5" (
    call :print_section "🔔 데이터 갱신 상태 테스트"
    if "%test_date%"=="" if "%test_time%"=="" (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type status
    ) else (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type status --test-date "%test_date%" --test-time "%test_time%"
    )
) else if "%test_choice%"=="6" (
    call :print_section "🎯 전체 테스트"
    if "%test_date%"=="" if "%test_time%"=="" (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type all
    ) else (
        python Monitoring/POSCO_News_250808/posco_main_notifier.py --test --test-type all --test-date "%test_date%" --test-time "%test_time%"
    )
