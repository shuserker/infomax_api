...
posco_monitoring() {
    ...
    if [[ -f "posco_control_center.sh" ]]; then
        bash .naming_backup/scripts/posco_control_center.sh"
# BROKEN_REF:     elif [[ -f "POSCO_통합_관리_센터_v3.bat" ]]; then
        ...
        print_info ".naming_backup/scripts/posco_control_center.sh"
        echo
        read -p "계속하려면 Enter를 누르세요..."
    fi
...
integrated_logs() {
    ...
    if [[ -d "$log_dir" ]]; then
        ...
        # 로그 파일 목록
# BROKEN_REF:         local log_files=($(find "$log_dir" -name "*.log" -type f 2>/dev/null))
...
# BROKEN_REF:         find "$LOG_DIR" -name "*.log" -mtime +30 -delete 2>/dev/null
