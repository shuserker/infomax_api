...
export const SystemMetricCard: React.FC<SystemMetricCardProps> = ({
  ...
  const renderTrendChart = () => {
    if (!showTrend || data.trend.length === 0 || type === 'network') return null;
    ...
    >
      ...
      <CardBody pt={0}>
        <VStack spacing={4} align="stretch">
          ...
          <VStack spacing={2} align="stretch">
            <HStack justify="space-between">
              <VStack align="start" spacing={0}>
                <Text fontSize="2xl" fontWeight="bold" color={valueColor}>
                  {isLoading ? '...' : 
                    type === 'network' ? data.unit : `${data.value.toFixed(1)}${data.unit}`
                  }
                </Text>
                {type === 'disk' && data.used_gb !== undefined && data.total_gb !== undefined && (
                  <Text fontSize="sm" color={textColor}>
                    {data.used_gb.toFixed(1)} GB / {data.total_gb.toFixed(1)} GB
                  </Text>
                )}
              </VStack>
              {data.threshold && type !== 'network' && (
                <Tooltip
            ...
            {type !== 'network' ? (
              ...
              <Box
                height="8px"
                borderRadius="md"
                bg={data.status === 'normal' ? 'green.500' : 'red.500'}
                width="100%"
              />
            )}
          ...
          {type === 'network' ? (
            <VStack spacing={1} align="stretch" fontSize="sm" color={textColor}>
              <HStack justify="space-between">
                <Text>연결 상태</Text>
                <Text color={data.status === 'normal' ? 'green.500' : 'red.500'} fontWeight="medium">
                  {data.unit}
                </Text>
              </HStack>
              <HStack justify="space-between">
