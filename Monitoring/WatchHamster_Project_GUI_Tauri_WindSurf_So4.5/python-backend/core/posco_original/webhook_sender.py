...
"""
    ...
    def send_watchhamster_status(self, status_message: str, status_details: Optional[Dict] = None,
        ...
        try:
            ...
            # 상세 상태가 있으면 그대로 사용, 없으면 기본 포맷
            if status_details and 'detailed_status' in status_details:
                formatted_message = status_details['detailed_status']
            else:
...
"""
                ...
                if status_details:
                    ...
                    for key, value in status_details.items():
                        if key != 'detailed_status':  # detailed_status는 이미 처리됨
                            formatted_message += f"  • {key}: {value}\n"
