...
async def lifespan(app: FastAPI):
    ...
    # WebSocket 백그라운드 태스크 시작 (테스트 환경에서는 비활성화)
...
app = FastAPI(
    ...
    openapi_tags=[
        {
            "name": "system",
            "description": "시스템 제어 API - 전체 시스템 시작/중지/재시작 및 상태 조회"
        },
        {
            "name": "services",
        ...
        {
            "name": "news",
            "description": "뉴스 모니터링 API - POSCO 뉴스 상태 조회, 갱신, 이력 관리"
        },
        {
            "name": "settings",
            "description": "설정 관리 API - 시스템 설정 조회, 업데이트, 백업, 복원"
        },
        {
            "name": "metrics",
...
# 보안 미들웨어 설정 (테스트 환경에서는 비활성화)
if not settings.debug and not os.getenv("TESTING"):
    app.add_middleware(
...
cors_origins = [
    "http://localhost:1420",  # Vite 개발 서버
    "http://127.0.0.1:1420",
...
router_configs = [
    ...
    ("api.webhooks", "/api/webhooks", "webhooks"),
    ("api.websocket", "/ws", "websocket"),
    ("api.posco", "/api/posco", "posco"),
    ("api.logs", "/api/logs", "logs"),
    ("api.diagnostics", "/api/diagnostics", "diagnostics"),
    ("api.monitor_logs", "/api/monitor-logs", "monitor-logs"),
    ("api.config_manager", "/api/config", "config"),
    ("api.webhook_manager", "/api/webhook-manager", "webhook-manager"),
    ("api.dashboard", "/api/dashboard", "dashboard"),  # 대시보드 API
    ("api.publish", "/api/publish", "publish")  # 배포 상태 API
]
...
for module_name, prefix, tag in router_configs:
    try:
        ...
        logger.info(f"✅ {module_name} 라우터 등록 성공")
    except Exception as e:
        logger.warning(f"❌ {module_name} 라우터 등록 실패: {e}")
