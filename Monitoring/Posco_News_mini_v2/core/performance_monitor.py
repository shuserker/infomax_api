...
# -*- coding: utf-8 -*-
"""
Performance Monitor for POSCO WatchHamster v2 System

v2 시스템을 위한 성능 모니터링 및 최적화 시스템
- CPU 및 메모리 사용량 추적
- v1과 v2 시스템 간 성능 비교
- 프로세스 관리 작업 응답 시간 모니터링
- 성능 알림 및 최적화 권장사항
...
class SystemPerformanceSnapshot:
    ...
    system_load_avg: Optional[Tuple[float, float, float]] = None
...
class PerformanceMonitor:
    ...
    def __init__(self, script_dir: str, monitoring_interval: int = 60):
        ...
        # 성능 데이터 저장소
        self.metrics_history: deque = deque(maxlen=1440)  # 24시간 분량 (1분 간격)
        self.process_metrics: Dict[str, deque] = defaultdict(lambda: deque(maxlen=60))  # 1시간 분량
        self.response_times: Dict[str, deque] = defaultdict(lambda: deque(maxlen=100))  # 최근 100회
    ...
    def record_response_time(self, operation_name: str, response_time: float):
        ...
        try:
            ...
            self.logger.debug(f"응답 시간 기록: {operation_name} = {response_time:.2f}초")
    ...
    def export_performance_data(self, filepath: str = None) -> str:
        ...
        try:
            if not filepath:
                ...
                filepath = os.path.join(self.script_dir, f'performance_data_{timestamp}.json')
...
class PerformanceComparator:
    ...
    def generate_comparison_report(self, v1_data: Dict[str, Any], v2_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            report_lines.append("🔍 POSCO 워치햄스터 v1/v2 성능 비교 보고서")
            report_lines.append("=" * 60)
