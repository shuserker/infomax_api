...
# -*- coding: utf-8 -*-
"""
WatchHamster Integration Module

기존 워치햄스터 코드와 새로운 아키텍처 컴포넌트들을 통합
- ProcessManager, ModuleRegistry, NotificationManager 통합
- 기존 알림 텍스트 내용 완전 보존
- 새로운 아키텍처 적용
...
# 새로운 컴포넌트들 import
from .enhanced_process_manager import ProcessManager, ProcessInfo, ProcessStatus
from .module_registry import ModuleRegistry, ModuleConfig, ModuleStatus
from .notification_manager import NotificationManager, SystemStatus, NotificationType
...
class EnhancedWatchHamster:
    ...
    def __init__(self, script_dir: str, webhook_url: str, bot_profile_url: str):
        ...
        self.module_registry = ModuleRegistry(os.path.join(script_dir, "modules.json"))
        self.notification_manager = NotificationManager(webhook_url, bot_profile_url)
        
        # 기존 컴포넌트 초기화 (호환성)
        if StateManager:
            status_file = os.path.join(script_dir, "WatchHamster_status.json")
    ...
    def stop_all_processes(self) -> bool:
        ...
        try:
            ...
            return len(stopped_processes) == len(running_processes)
