...
class NewsStatus(BaseModel):
    ...
    def validate_data_type(cls, v, values):
        ...
        if news_type == NewsType.EXCHANGE_RATE and not isinstance(v, ExchangeRateData):
            raise ValueError("Exchange rate news must have ExchangeRateData")
        elif news_type == NewsType.NEWYORK_MARKET and not isinstance(v, NewYorkMarketData):
            raise ValueError("New York market news must have NewYorkMarketData")
        elif news_type == NewsType.KOSPI_CLOSE and not isinstance(v, KospiCloseData):
...
class NewsStatusSummary(BaseModel):
    ...
    overall_status: str = Field(..., regex="^(healthy|warning|critical)$", description="전체 상태")
...
class NewsAlert(BaseModel):
    ...
    type: NewsType = Field(..., description="뉴스 타입")
    alert_type: str = Field(..., regex="^(delay|error|recovery|status_change)$", description="알림 타입")
