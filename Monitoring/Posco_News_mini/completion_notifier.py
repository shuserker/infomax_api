...
# -*- coding: utf-8 -*-
"""
POSCO 리포트 시스템 완료 알림자

작업 완료 후 Dooray를 통해 결과 알림을 전송하는 클래스
리포트 관련 작업 완료만 알림하며, 다른 시스템 알림은 건드리지 않음
"""
...
- 📱 개선된 메타데이터 관리 시스템
        ...
        if successful_reports:
            ...
            if len(successful_reports) > 5:
                summary += f"\n- ... 외 {len(successful_reports) - 5}개 더"
        
        # 오류 정보 추가
        failed_reports = [r for r in generation_results if r.get('status') == 'failed']
        if failed_reports:
            summary += f"\n\n⚠️ 주의사항: {len(failed_reports)}개 리포트 생성 실패"
    ...
    def create_dashboard_links(self, results: Dict[str, Any] = None) -> Dict[str, str]:
        ...
        if results and results.get('generation_results'):
            ...
            docs_reports_path = "docs/reports/posco_integrated_analysis_*.html"
            monitoring_reports_path = "Monitoring/Posco_News_mini/reports/posco_integrated_analysis_*.html"
        ...
        return {
            ...
            'reports_api': f"{base_url}/docs/reports_index.json",
