...
증시마감(KOSPI) 뉴스를 전문적으로 모니터링하는 시스템

주요 기능:
- 증시마감 뉴스 실시간 모니터링
- 발행 패턴 분석 및 예측 (15:40 기준)
- 지연 발행 감지 및 알림
- 과거 데이터와의 비교 분석
- 자동 알림 및 상태 보고
...
특화 기능:
- 15:40 정시 발행 패턴 추적
- 지연 발행 시 자동 알림 (16:00, 16:30, 17:00)
- 주말/공휴일 발행 예외 처리
- 증시 변동성 기반 중요도 판단
...
class KospiCloseMonitor:
    ...
    증시마감 뉴스의 특성을 고려한 전문 모니터링 시스템입니다.
    
    주요 기능:
    - 정시 발행 패턴 추적 (평일 15:40)
    - 지연 발행 감지 및 단계별 알림
    - 증시 변동성 분석
    - 과거 데이터 비교 분석
    """
    ...
    def __init__(self):
        ...
        self.display_name = self.news_config.get('display_name', '증시마감')
        self.emoji = self.news_config.get('emoji', '📈')
        
        # 발행 시간 패턴 (평일 15:40 기준)
        self.expected_publish_time = "154000"  # 15:40:00
        self.delay_check_times = ["160000", "163000", "170000"]  # 16:00, 16:30, 17:00
    ...
    def get_current_kospi_data(self):
        """
        현재 증시마감 뉴스 데이터 조회
    ...
    def analyze_publish_pattern(self, kospi_data):
        ...
        is_on_time = abs(delay_minutes) <= 10
    ...
    def send_delay_notification(self, delay_stage):
        ...
        stage_times = {1: "16:00", 2: "16:30", 3: "17:00"}
        ...
        message += f"📋 예상 발행 시간: 15:40\n"
        message += f"🚨 지연 상태: {expected_time} 기준 미발행\n\n"
        
        # 지연 단계별 메시지
        if delay_stage == 1:
            message += "• 20분 지연 상태입니다.\n"
            message += "• 증시마감 일반적인 지연 범위입니다.\n"
        elif delay_stage == 2:
            message += "• 50분 지연 상태입니다.\n"
            message += "• 주의가 필요한 지연입니다.\n"
        else:
            message += "• 1시간 20분 이상 지연 상태입니다.\n"
    ...
    def send_publish_notification(self, kospi_data, pattern_analysis):
        """
        증시마감 발행 알림 전송
        ...
        """
        title = kospi_data.get('title', '')
        date = kospi_data.get('date', '')
        time = kospi_data.get('time', '')
    ...
    def run_single_check(self):
        ...
        # 현재 데이터 조회
        kospi_data = self.get_current_kospi_data()
        ...
        # 발행 패턴 분석
        pattern_analysis = self.analyze_publish_pattern(kospi_data)
        
        print(f"📊 분석 결과: {pattern_analysis.get('analysis', '분석 불가')}")
        
        # 변경사항 감지
        if self.last_data != kospi_data:
            print(f"🆕 {self.display_name} 데이터 변경 감지")
            
            # 발행 알림 전송
            if pattern_analysis.get('is_published_today', False):
                self.send_publish_notification(kospi_data, pattern_analysis)
            
            self.last_data = kospi_data.copy() if kospi_data else None
        else:
