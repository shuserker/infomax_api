...
class LogAggregation(BaseModel):
    ...
    time_interval: Optional[str] = Field(None, regex="^(1m|5m|15m|1h|6h|24h)$", description="시간 간격")
...
class LogAlertRule(BaseModel):
    ...
    operator: str = Field(..., regex="^(equals|contains|greater_than|less_than|regex)$", description="연산자")
...
class LogAlertCondition(BaseModel):
    ...
    logic: str = Field(default="AND", regex="^(AND|OR)$", description="논리 연산자")
...
class LogExportJob(BaseModel):
    ...
    status: str = Field(..., regex="^(pending|running|completed|failed|cancelled)$", description="작업 상태")
...
class GetLogStatisticsRequest(BaseModel):
    """로그 통계 조회 요청 모델"""
    filter: Optional[LogFilter] = Field(None, description="로그 필터")
    period: str = Field(default="24h", regex="^(1h|6h|24h|7d|30d)$", description="통계 기간")
...
class LogStreamMessage(BaseModel):
    """로그 스트림 메시지 모델"""
    type: str = Field(..., regex="^(logs|status|error)$", description="메시지 타입")
