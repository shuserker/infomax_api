...
# 핵심 모듈 임포트 - 기존 WatchHamster_Project 로직 사용
...
def get_api_client():
    """API 클라이언트 인스턴스 반환 - 기존 WatchHamster_Project 로직 사용"""
    global api_client
    if not api_client:
        # 기존 WatchHamster_Project의 설정 구조 사용
        api_config = {
            "url": "https://global-api.einfomax.co.kr/apis/posco/news",
            "user": "your_api_user",  # 환경변수에서 로드해야 함
            "password": "your_api_password",  # 환경변수에서 로드해야 함
            "timeout": 30,
            "max_retries": 3,
            "retry_delay": 1.0
        }
        api_client = InfomaxAPIClient(api_config)
    return api_client

def get_news_parser():
    """뉴스 파서 인스턴스 반환 - 기존 WatchHamster_Project 로직 사용"""
    global news_parser
    if not news_parser:
        # 기존 WatchHamster_Project의 통합 뉴스 파서 사용
        news_parser = IntegratedNewsParser()
    return news_parser

@router.get("/status")
async def get_news_status(news_type: Optional[str] = Query(None, description="특정 뉴스 타입 조회")):
    """뉴스 상태 조회 - 기존 WatchHamster_Project 로직 사용"""
    ...
    try:
        ...
        if news_type:
            # 특정 뉴스 타입 조회
            if news_type not in ["exchange-rate", "newyork-market-watch", "kospi-close"]:
                raise HTTPException(status_code=404, detail=f"뉴스 타입을 찾을 수 없습니다: {news_type}")
            
            # 실제 뉴스 데이터 파싱 및 상태 판단
            try:
                # 기존 WatchHamster_Project의 로직으로 뉴스 데이터 처리
                result = await parser.parse_news_data_by_type(news_type)
                
                return NewsStatus(
                    type=news_type,
                    status=result.get("status", "unknown"),
                    last_update=result.get("last_update", datetime.now()),
                ...
                logger.error(f"뉴스 타입 {news_type} 처리 실패: {e}")
                return NewsStatus(
                    type=news_type,
                    status="error",
            ...
            for ntype in news_types:
                try:
                    # 기존 WatchHamster_Project의 로직으로 뉴스 데이터 처리
                    parsed_result = await parser.parse_news_data_by_type(ntype)
                    
                    result.append(NewsStatus(
                        type=ntype,
                        status=parsed_result.get("status", "unknown"),
                        last_update=parsed_result.get("last_update", datetime.now()),
                        expected_time=parsed_result.get("expected_time"),
                        delay_minutes=parsed_result.get("delay_minutes", 0),
                        data=parsed_result.get("data"),
                        error_message=parsed_result.get("error_message")
                    ))
                except Exception as e:
                    logger.error(f"뉴스 타입 {ntype} 처리 실패: {e}")
                    result.append(NewsStatus(
                        type=ntype,
                        status="error",
                        last_update=datetime.now(),
...
async def refresh_news_data(
    ...
    """뉴스 데이터 수동 갱신 - 기존 WatchHamster_Project 로직 사용"""
    logger.info(f"뉴스 데이터 갱신 요청: {request.news_types}, force={request.force}")
    
    try:
        # 갱신할 뉴스 타입 결정
        valid_news_types = ["exchange-rate", "newyork-market-watch", "kospi-close"]
        news_types_to_refresh = request.news_types or valid_news_types
        
        # 유효한 뉴스 타입인지 확인
        invalid_types = [nt for nt in news_types_to_refresh if nt not in valid_news_types]
        if invalid_types:
            raise HTTPException(
                status_code=400, 
        ...
        background_tasks.add_task(_refresh_news_task_with_real_api, news_types_to_refresh, request.force)
        
        return {
            "message": f"뉴스 데이터 갱신 중... (기존 WatchHamster_Project 로직 사용)",
...
# 실제 API를 사용하는 백그라운드 태스크 함수
async def _refresh_news_task_with_real_api(news_types: List[str], force: bool = False):
    """실제 INFOMAX API를 사용하여 뉴스 데이터 갱신 - 기존 WatchHamster_Project 로직 사용"""
    logger.info(f"실제 API를 사용한 뉴스 데이터 갱신 시작: {news_types}, force={force}")
    
    try:
        # 기존 WatchHamster_Project의 API 클라이언트와 파서 사용
        api_client = get_api_client()
        parser = get_news_parser()
        
        for news_type in news_types:
            try:
                logger.info(f"뉴스 타입 {news_type} 갱신 시작")
                
                # 실제 INFOMAX API 호출
                raw_data = await api_client.fetch_news_data(news_type)
                
                if raw_data:
                    # 기존 WatchHamster_Project의 파싱 로직 사용
                    parsing_result = parser.parse_all_news_data({news_type: raw_data})
                    parsed_result = {
                        "status": "latest" if parsing_result.success else "error",
                        "last_update": datetime.now(),
                        "data": parsing_result.data.__dict__ if parsing_result.data else None,
                        "error_message": "; ".join(parsing_result.errors) if parsing_result.errors else None
                    }
                    
                    # 결과를 전역 저장소에 업데이트
                    news_status_store[news_type] = {
                        "status": parsed_result.get("status", "unknown"),
                        "last_update": datetime.now(),
                        "expected_time": parsed_result.get("expected_time"),
                        "delay_minutes": parsed_result.get("delay_minutes", 0),
                    ...
                    # 이력에 추가
                    history_entry = {
                        "id": f"{news_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                        "type": news_type,
                        "timestamp": datetime.now(),
                        "status": parsed_result.get("status", "unknown"),
                        "data": parsed_result.get("data", {}),
                    ...
                    # 이력 저장소 크기 제한 (최근 100개만 유지)
                    if len(news_history_store) > 100:
                        news_history_store.pop(0)
                    
                    logger.info(f"뉴스 타입 {news_type} 갱신 완료: {parsed_result.get('status', 'unknown')}")
                else:
                    logger.warning(f"뉴스 타입 {news_type}에 대한 데이터를 가져올 수 없습니다")
                    news_status_store[news_type] = {
                        "status": "error",
                        "last_update": datetime.now(),
                        "error_message": "API에서 데이터를 가져올 수 없습니다"
                ...
                logger.error(f"뉴스 타입 {news_type} 갱신 실패: {e}")
                news_status_store[news_type] = {
                    "status": "error",
                    "last_update": datetime.now(),
                    "error_message": str(e)
        ...
        logger.info("실제 API를 사용한 뉴스 데이터 갱신 완료")
