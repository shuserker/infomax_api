...
class IntegratedNewsParser:
    ...
    async def parse_news_data_by_type(self, news_type: str) -> Dict[str, Any]:
        """특정 뉴스 타입별 데이터 파싱 - API 호환성을 위한 래퍼 메서드"""
        try:
            # 더미 데이터로 테스트 (실제 API 연동 전)
            dummy_data = {
                "type": news_type,
                "status": "latest",
                "last_update": datetime.now().isoformat(),
                "data": {
                    "title": f"{news_type} 뉴스 데이터",
                    "content": f"이것은 {news_type}에 대한 테스트 데이터입니다.",
                    "timestamp": datetime.now().isoformat()
                }
            }
            
            self.logger.info(f"뉴스 타입 {news_type} 파싱 완료 (더미 데이터)")
            return dummy_data
            
        except Exception as e:
            self.logger.error(f"뉴스 타입 {news_type} 파싱 실패: {e}")
            return {
                "type": news_type,
                "status": "error",
                "last_update": datetime.now().isoformat(),
