...
export const ServiceInfoSchema = z.object({
  ...
  status: ServiceStatusSchema.or(z.enum(['unknown'])),
...
export const SystemMetricsSchema = z.object({
  ...
  memory_available: z.number().min(0),
  disk_usage: z.number().min(0).max(100),
  disk_used: z.number().min(0),
  disk_free: z.number().min(0),
...
export const WebhookTemplateSchema = z.object({
  ...
  message_type: z.enum(['custom', 'news_alert', 'system_status', 'service_alert', 'error_report']),
