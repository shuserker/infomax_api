...
class DashboardSettings(BaseModel):
    """대시보드 설정 모델"""
    layout: str = Field(default="grid", pattern="^(grid|list|compact)$", description="레이아웃")
...
class NotificationUISettings(BaseModel):
    ...
    position: str = Field(default="top-right", pattern="^(top-right|top-left|bottom-right|bottom-left)$", description="알림 위치")
...
class FileLoggingSettings(BaseModel):
    ...
    rotation: str = Field(default="daily", pattern="^(daily|weekly|size-based)$", description="로테이션 방식")
...
class AutoBackupSettings(BaseModel):
    ...
    interval: str = Field(default="daily", pattern="^(daily|weekly|monthly)$", description="백업 간격")
    time: str = Field(default="02:00", pattern=r"^([01]?[0-9]|2[0-3]):[0-5][0-9]$", description="백업 시간 (HH:MM)")
...
class SettingsValidationError(BaseModel):
    ...
    severity: str = Field(..., pattern="^(error|warning)$", description="심각도")
...
class ExportSettingsRequest(BaseModel):
    ...
    include_sensitive: bool = Field(default=False, description="민감 정보 포함")
    format: str = Field(default="json", pattern="^(json|yaml)$", description="내보내기 형식")
...
class SettingsTemplate(BaseModel):
    ...
    category: str = Field(..., pattern="^(development|production|testing|custom)$", description="카테고리")
    settings: Dict[str, Any] = Field(..., description="설정 데이터")
...
class SettingsConflict(BaseModel):
    ...
    resolution: str = Field(..., pattern="^(local|remote|manual|pending)$", description="해결 방법")
