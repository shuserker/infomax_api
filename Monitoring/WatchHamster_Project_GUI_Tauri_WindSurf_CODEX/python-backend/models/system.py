...
class ResourceUsage(BaseModel):
    ...
    status: str = Field(..., pattern="^(normal|warning|critical)$", description="상태")
...
class ServiceControl(BaseModel):
    ...
    action: str = Field(..., pattern="^(start|stop|restart|reload|status)$", description="액션")
...
class ControlServiceRequest(BaseModel):
    """서비스 제어 요청 모델"""
    service_id: str = Field(..., description="서비스 ID")
    action: str = Field(..., pattern="^(start|stop|restart)$", description="액션")
    force: bool = Field(default=False, description="강제 실행")
...
class HealthCheck(BaseModel):
    ...
    status: str = Field(..., pattern="^(healthy|unhealthy|degraded)$", description="상태")
...
class StabilityMetrics(BaseModel):
    ...
    system_health: str = Field(..., pattern="^(healthy|warning|critical)$", description="시스템 상태")
