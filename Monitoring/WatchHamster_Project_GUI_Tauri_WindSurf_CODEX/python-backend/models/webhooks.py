...
class WebhookQueueItem(BaseModel):
    ...
    priority: str = Field(default="normal", pattern="^(low|normal|high|critical)$", description="우선순위")
...
class WebhookEvent(BaseModel):
    """웹훅 이벤트 모델"""
    type: str = Field(..., pattern="^(webhook_sent|webhook_failed|webhook_retry|webhook_cancelled)$", description="이벤트 타입")
    webhook_id: str = Field(..., description="웹훅 ID")
    message_id: str = Field(..., description="메시지 ID")
...
class WebhookPayload(BaseModel):
    ...
    webhook_type: str = Field(default="dooray", pattern="^(dooray|discord|slack|generic)$", description="웹훅 타입")
