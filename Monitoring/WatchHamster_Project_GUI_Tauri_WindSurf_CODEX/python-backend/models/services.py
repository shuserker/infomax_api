...
class ServiceHealthCheck(BaseModel):
    """서비스 헬스 체크 모델"""
    service_id: str = Field(..., description="서비스 ID")
    status: str = Field(..., pattern="^(healthy|unhealthy|degraded|unknown)$", description="헬스 상태")
...
class ServiceDependency(BaseModel):
    ...
    dependency_type: str = Field(default="required", pattern="^(required|optional|weak)$", description="의존성 타입")
    created_at: datetime = Field(default_factory=datetime.now, description="생성 시간")
...
class ServiceEvent(BaseModel):
    ...
    severity: str = Field(..., pattern="^(info|warning|error|critical)$", description="심각도")
...
class ServiceBackup(BaseModel):
    ...
    service_id: str = Field(..., description="서비스 ID")
    backup_type: str = Field(..., pattern="^(config|logs|data|full)$", description="백업 타입")
...
class ServiceRestoreRequest(BaseModel):
    ...
    restore_type: str = Field(..., pattern="^(config|logs|data|full)$", description="복원 타입")
...
class ServiceAction(BaseModel):
    """서비스 액션 모델 (레거시 호환성)"""
    action: str = Field(..., pattern="^(start|stop|restart)$", description="수행할 액션")
    service_id: str = Field(..., description="대상 서비스 ID")
    parameters: Optional[Dict[str, Any]] = Field(None, description="액션 매개변수")
