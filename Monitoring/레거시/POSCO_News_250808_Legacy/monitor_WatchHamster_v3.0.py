...
try:
    from .git/config import .naming_backup/config_data_backup/watchhamster.log, BOT_PROFILE_IMAGE_URL, API_CONFIG
from core import posco_news_250808_monitor.log News 250808DataProcessor, DoorayNotifier
# REMOVED:     from Monitoring/POSCO_News_250808/core/process_manager.py import ProcessManager
    
    # StateManager 직접 import
# REMOVED:     from Monitoring/POSCO_News_250808/core/state_manager.py import StateManager
    try:
# REMOVED:         from Monitoring/POSCO_News_250808/core/colorful_ui.py import ColorfulConsoleUI
    except ImportError:
        ColorfulConsoleUI = None
    # 최적화된 모니터링 시스템 연결
    try:
# REMOVED:         from Monitoring/POSCO_News_250808/backup_archive_20250806/disabled_monitors_20250803/newyork_monitor.py import NewYorkMarketMonitor
    except ImportError:
        NewYorkMarketMonitor = None
        print("[INFO] NewYorkMarketMonitor 비활성화됨 - 통합 리포트 시스템 사용")
    
    try:
# REMOVED:         from Monitoring/POSCO_News_250808/backup_archive_20250806/disabled_monitors_20250803/kospi_monitor.py import KospiCloseMonitor
    except ImportError:
        KospiCloseMonitor = None
        print("[INFO] KospiCloseMonitor 비활성화됨")
    
    try:
# REMOVED:         from Monitoring/POSCO_News_250808/backup_archive_20250806/disabled_monitors_20250803/exchange_monitor.py import ExchangeRateMonitor
    except ImportError:
        ExchangeRateMonitor = None
        print("[INFO] ExchangeRateMonitor 비활성화됨")
    
    try:
# REMOVED:         from Monitoring/POSCO_News_250808/backup_archive_20250806/disabled_monitors_20250803/master_news_monitor.py import MasterNewsMonitor
    except ImportError:
...
class WatchHamster v3.00Monitor:
    ...
    def __init__(self):
        ...
        # 통합 리포트 스케줄러 초기화
        try:
# REMOVED:             from Monitoring/POSCO_News_250808/integrated_report_scheduler.py import IntegratedReportScheduler
    ...
    def _initialize_v3_0_components(self):
        ...
        구현 내용:
# REMOVED:         - 동적 import 로직으로 v2 컴포넌트 로드
...
# REMOVED:             raise ImportError(f"ModuleRegistry import 실패: {e}")
        except Exception as e:
...
# REMOVED:             raise ImportError(f"NotificationManager import 실패: {e}")
        except Exception as e:
...
# REMOVED:             self.log(f"⚠️ 성능 모니터링 모듈 import 실패: {e}")
            self.performance_monitor = None
            self.performance_optimizer = None
            self.performance_comparator = None
        except Exception as e:
    ...
    def handle_process_failure_v3_0(self, process_name: str):
        ...
        try:
            ...
            if recovery_success:
                ...
                if self.v3_0_components['module_registry']:
                    try:
# REMOVED:                         from Monitoring/WatchHamster_v3.0/core/module_registry.py import docs/status.json
...
# REMOVED:             from WatchHamster_v3.0.log import ProcessInfo, ProcessStatus
    ...
    def _create_system_status_object_v3_0(self, current_time, health_results):
        ...
        """
        try:
# REMOVED:             from Monitoring/WatchHamster_v3.0/core/notification_manager.py import docs/status.json
        ...
        try:
            ...
            if self.is_quiet_hours():
                ...
                if has_problem:
                    ...
                    # 정상 상태: 간단한 알림
                    self.send_notification(
                        f"🌙 POSCO WatchHamster v3.0 정기 보고 (조용한 시간)/n/n"
                        f"📅 시간: {current_time.strftime('%Y-%m-%d %H:%M:%S')}/n"
                        f"🔍 모니터링: {monitor_status}/n"
                        f"📊 {resource_info}/n"
            ...
            self.send_notification(
                f"❌ POSCO WatchHamster v3.0 정기 보고 오류/n/n"
                f"📅 시간: {current_time.strftime('%Y-%m-%d %H:%M:%S')}/n"
                f"❌ 오류: {str(e)}/n"
                f"🔧 수동 확인이 필요합니다.",
                is_error=True
    ...
    def send_notification(self, message, is_error=False):
        ...
        try:
            ...
            if self.v3_0_enabled and self.v3_0_components['notification_manager']:
                success = self.v3_0_components['notification_manager'].send_notification(message, is_error)
                if success:
                    return
                else:
...
#_간격_계산:_(현재시간_-_시작시간)_%_간격 = = 0
    ...
    def send_status_notification(self):
        ...
        try:
            ...
            self.send_notification(
                f"❌ POSCO WatchHamster v3.0 🛡️ 상태 알림 오류/n/n"
                f"📅 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}/n"
                f"❌ 오류: {str(e)}/n"
                f"🔧 수동 확인이 필요합니다.",
                is_error=True
        ...
        try:
            ...
            if is_quiet:
                ...
                if has_problem:
                    ...
                    self.send_notification(
                        f"🚨 POSCO WatchHamster v3.0 매시간 체크 - 중요 문제 감지/n/n"
                        f"📅 시간: {current_time.strftime('%Y-%m-%d %H:%M:%S')}/n"
                        f"🚨 감지된 문제:/n" + "/n".join(f"   • {problem}" for problem in problem_details) + "/n/n"
                        f"🔍 상세 상태:/n"
                        f"   • 모니터링: {monitor_status}/n"
                        f"   • API: {api_status}/n"
                        f"   • {resource_info}/n"
                        f"⏰ 다음 체크: {(current_hour + 1) % 24:02d}:00/n"
                        f"🔧 자동 복구 시도 중...",
                        is_error=True
                    )
                ...
                if self.smart_enabled and smart_status_info and current_data:
                    ...
                    try:
                        ...
                        # 기본 알림으로 대체
                        self.send_notification(
                            f"🕐 POSCO WatchHamster v3.0 매시간 상태 체크/n/n"
                            f"📅 시간: {current_time.strftime('%Y-%m-%d %H:%M:%S')}/n"
                            f"🔍 모니터링 프로세스: {monitor_status}/n"
                            f"🌐 API 연결: {api_status}/n"
                            f"📊 {resource_info}/n"
    ...
    def send_enhanced_status_notification(self):
        ...
        try:
            ...
            message = f"🛡️ POSCO WatchHamster v3.0 향상된 상태 보고/n/n"
            message += f"📅 시간: {current_time.strftime('%Y-%m-%d %H:%M:%S')}/n"
            message += f"🔍 모니터링 프로세스: {monitor_status}/n/n"
            
            # 개별 모니터 상태 추가
            individual_monitors = enhanced_status.get('individual_monitors', {})
            if individual_monitors:
                message += f"📊 개별 뉴스 모니터 상태:/n"
                ...
                for news_type, info in individual_monitors.items():
                    ...
                    message += f"   {status_emoji} {name}: {analysis}/n"
                ...
                message += f"/n📈 전체 발행 현황: {published_count}/{total_count} 완료/n"
            
            # 시스템 통합 상태
            if self.master_monitor_enabled:
                message += f"/n🎛️ 마스터 모니터링: 연동 활성화"
            if self.individual_monitors_enabled:
                message += f"/n🔧 개별 모니터링: 연동 활성화"
            
            message += f"/n⏰ 다음 보고: {(current_time + timedelta(hours=2)).strftime('%H:%M')}"
