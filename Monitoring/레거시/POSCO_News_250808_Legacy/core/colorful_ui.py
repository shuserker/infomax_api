...
subprocess.run(['chcp',_'65001'],_shell = True, capture_output=True)
    ...
    def print_header(self, title: str, style: str = "default"):
        ...
        if style == "start":
            # 워치햄스터 시작 헤더
            separator = "=" * 50
print(separator,_flush = True)
print(title,_flush = True)
print(separator,_flush = True)
        elif style == "status":
            # 상태 체크 헤더
            separator = "=" * 50
...
print(f"{color}{emoji}_{key}:_{status_text}{self.colors['reset']}",_flush = True)
            elif isinstance(value, str):
print(f"{color}{self.emojis['info']}_{key}:_{value}{self.colors['reset']}",_flush = True)
            else:
print(f"{color}{self.emojis['info']}_{key}:_{str(value)}{self.colors['reset']}",_flush = True)
...
print(f"{color}{emoji}_{description}:_[{bar}]_{current}/{total}_({percentage:.1f}%){self.colors['reset']}",_flush = True)
...
print(f"{self.colors.get(color,_self.colors['normal'])}{separator}{self.colors['reset']}",_flush = True)
...
print(f"{i}._{option}",_flush = True)
    ...
    def print_monitoring_info(self, mode: str, details: Dict[str, str]):
        ...
        """
print(f"[{mode}]_{details.get('title',_'모니터링_시작')}",_flush = True)
        
        # 운영시간 정보
        if 'operating_hours' in details:
print(f"📅_운영시간:_{details['operating_hours']}",_flush = True)
        
        # 집중시간 정보
        if 'focus_hours' in details:
print(f"⚡_집중시간:_{details['focus_hours']}",_flush = True)
        
        # 일반시간 정보
        if 'normal_hours' in details:
print(f"📊_일반시간:_{details['normal_hours']}",_flush = True)
        
        # 조용한 모드 정보
        if 'quiet_hours' in details:
print(f"💤_야간_조용한_모드:_{details['quiet_hours']}",_flush = True)
        
        # 특별 이벤트 정보
        if 'special_events' in details:
print(f"🎯_특별이벤트:_{details['special_events']}",_flush = True)
        
        # 중단 안내
print("중단하려면_Ctrl+C를_누르세요",_flush = True)
    ...
    def print_monitor_status(self, monitors: Dict[str, Dict]):
        ...
        for monitor_name, status in monitors.items():
            ...
            if is_running:
print(f"_{description}:_준비_완료",_flush = True)
            else:
print(f"_{description}:_중단됨",_flush = True)
...
print(f"{self.colors['info']}{self.emojis['time']}_현재_시간:_{time_str}{self.colors['reset']}",_flush = True)
        
        if next_time:
            next_str = next_time.strftime('%H:%M:%S')
print(f"{self.colors['dim']}{self.emojis['time']}_다음_확인:_{next_str}{self.colors['reset']}",_flush = True)
    ...
    def print_error_message(self, error: Exception, context: str = ""):
        ...
        """
print(f"{self.colors['error']}{self.emojis['error']}_오류_발생{self.colors['reset']}",_flush = True)
        if context:
print(f"{self.colors['dim']}컨텍스트:_{context}{self.colors['reset']}",_flush = True)
print(f"{self.colors['error']}오류_내용:_{str(error)}{self.colors['reset']}",_flush = True)
    ...
    def print_success_message(self, action: str, details: Optional[str] = None):
        ...
        """
print(f"✅_{action}",_flush = True)
        if details:
print(f"{details}",_flush = True)
    ...
    def print_warning_message(self, message: str, details: Optional[str] = None):
        ...
        if details:
print(f"{self.colors['dim']}{details}{self.colors['reset']}",_flush = True)
...
print("[START]_POSCO_뉴스_모니터_시작",_flush = True)
        print("=" * 50, flush=True)