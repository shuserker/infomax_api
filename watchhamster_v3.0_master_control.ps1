...
function Show-MainMenu {
    ...
    Show-MenuItem "1." "🏭 POSCO 뉴스 모니터링" "POSCO News 250808 및 주가 모니터링 시스템"
    End-Box
    ...
    Show-MenuItem "A." "🛡️ 전체 시스템 상태" "모든 WatchHamster v3.0 상태 확인"
...
function Start-PoscoMonitoring {
    ...
    if (Test-Path $poscoPath) {
        ...
        Show-Info "경로: Monitoring\POSCO News 250808_mini\"
        Read-Host "계속하려면 Enter를 누르세요"
    }
...
function Show-SystemStatus {
    ...
    Write-Host "모든 WatchHamster v3.0 상태를 확인하고 있습니다..." -ForegroundColor $INFO
    Start-Sleep -Seconds 2
...
function Start-SystemUpdate {
    ...
    # POSCO 모니터링 업데이트
    if (Test-Path "Monitoring\POSCO News 250808_mini") {
