...
# 기존 워치햄스터 중지
if pgrep -f "Monitoring/POSCO_News_250808/Monitoring/POSCO_News_250808/monitor_WatchHamster_v3.0.py" > /dev/null; then
    print_warning "기존 워치햄스터 중지 중..."
    pkill -f "Monitoring/POSCO_News_250808/Monitoring/POSCO_News_250808/monitor_WatchHamster_v3.0.py"
    sleep 5
fi
...
# v2 디렉토리 생성
mkdir -p Monitoring/WatchHamster_v3.0/core
...
config_data = {
    ...
    "modules": {
        "posco_main_notifier": {
            ...
            "working_directory": "../POSCO News 250808_mini",
            "timeout": 30,
            "priority": 1
        },
...
# 선택적 모듈 확인 및 추가
optional_modules = ["Monitoring/POSCO_News_250808/Monitoring/POSCO_News_250808/historical_data_collector.py"]
for module_file in optional_modules:
    module_path = f"Monitoring/POSCO News 250808_mini/{module_file}"
...
# modules.json 생성
with open('Monitoring/WatchHamster_v3.0/modules.json', 'w', encoding='utf-8') as f:
...
# 새로운 아키텍처 import .naming_backup/scripts/🧪POSCO_테스트_실행.bat
python3 -c "
import system_functionality_verification.py
import posco_news_250808_monitor.log
sys.path.insert(0, 'Monitoring/POSCO News 250808_mini')

try:
    from .comprehensive_repair_backup/monitor_WatchHamster_v3.0.py.backup_20250809_181656 import .naming_backup/config_data_backup/watchhamster.log
    ...
    print(f'❌ WatchHamster v3.0 초기화 실패: {e}')
    exit(1)
"
...
if grep -q "워치햄스터 통합 관리" watchhamster_control_center.sh 2>/dev/null; then
    print_success "WatchHamster v3.0.0 메뉴 구조 확인"
else
    print_warning "WatchHamster v3.0.0으로 업데이트되지 않았습니다"
fi
...
timeout 10s python3 Monitoring/POSCO News 250808_mini/monitor_WatchHamster_v3.0.py &
TEST_PID=$!
...
echo ""
echo "🎉 POSCO WatchHamster v3.0.0 마이그레이션 완료!"
echo ""
echo "📋 마이그레이션 결과:"
echo "  ✅ 기존 기능 100% 보존"
echo "  ✅ 새로운 아키텍처 적용"
...
echo ""
echo "🚀 다음 단계:"
echo "  1. ./.naming_backup/scripts/watchhamster_control_center.sh 실행"
...
echo "run_migration_verification.sh"
echo "rollback_migration.sh"
echo ""