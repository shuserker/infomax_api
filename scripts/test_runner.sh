...
# BROKEN_REF:         if python3 -c "import $module" 2>/dev/null; then
    ...
    # 테스트 스크립트 확인
    local test_scripts=("test_watchhamster_v3.0_integration.py" "test_process_lifecycle.py" "test_control_center_integration.py" "run_comprehensive_tests.py")
...
run_comprehensive_tests() {
    print_header "🚀 POSCO WatchHamster v3.0 종합 테스트 실행"
...
# 도움말 표시
show_help() {
    print_header "📖 POSCO WatchHamster v3.0 테스트 러너 도움말"
...
main() {
    ...
    case "$command" in
        ...
        "v2")
            if check_environment; then
                run_individual_test "v2 통합 테스트" "test_watchhamster_v3.0_integration.py"
            else
                exit 1
            fi
            ;;
