...
# REMOVED: from unittest.mock import MagicMock, patch, Mock
import psutil
...
# REMOVED:         """v2 컴포넌트 import 테스트"""
        try:
            ...
            # modules.json 파일 확인
            modules_json = os.path.join(v2_path, '.naming_backup/config_data_backup/Monitoring/Posco_News_mini/modules.json')
            ...
            self.fail(f"v2 컴포넌트 import .naming_backup/scripts/🧪POSCO_테스트_실행.bat 실패: {e}")
...
# REMOVED:             from unittest.mock import MagicMock
    ...
    def test_v2_fallback_mechanism(self):
        ...
        try:
            ...
            v2_path = os.path.join(current_dir, 'Monitoring', 'WatchHamster_v3.0')
            temp_path = os.path.join(current_dir, 'Monitoring', 'WatchHamster_v3_v3_0_temp')
            
            # 실제 파일 시스템 조작 대신 mock을 사용
            import system_functionality_verification.py
# REMOVED:             from unittest.mock import MagicMock, patch
...
# REMOVED:             from unittest.mock import MagicMock, patch
            ...
            # v2 컴포넌트 mock
with_patch('os.path.exists',_return_value = True):
...
# REMOVED:                         module_config_mock.script_path = 'ci_test_report.md'
...
# REMOVED:         self.test_script_path = os.path.join(self.test_script_dir, 'ci_test_report.md')
...
with_self.subTest(scenario = scenario['name']):
...
class TestControlCenterIntegration(unittest.TestCase):
    ...
    def setUp(self):
        """테스트 설정"""
        self.control_center_script = os.path.join(current_dir, '.naming_backup/scripts/watchhamster_control_center.sh')
...
def run_comprehensive_tests():
    ...
    for test_class in test_classes:
        print(f"/n📋 {test_class.__name__} 실행 중...")
        print("-" * 60)
        ...
        # 결과 집계
total_tests_+ =  result.testsRun
passed_tests_+ =  result.testsRun - len(result.failures) - len(result.errors)
failed_tests_+ =  len(result.failures) + len(result.errors)
        
        # 실패한 테스트 상세 정보
        if result.failures or result.errors:
            print(f"/n❌ {test_class.__name__} 실패 상세:")
...
print("/n"_+_" = " * 80)
    ...
    if failed_tests == 0:
        ...
        print(f"/n⚠️ {failed_tests}개의 테스트가 실패했습니다.")
