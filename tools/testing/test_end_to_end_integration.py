...
class EndToEndSystemTest(unittest.TestCase):
    ...
    def create_mock_watchhamster_script(self) -> str:
        """Mock 워치햄스터 스크립트 생성"""
        script_path = os.path.join(self.test_dir, '.naming_backup/config_data_backup/watchhamster.log')
...
'''
    ...
    def test_full_system_startup_shutdown(self):
        ...
        try:
            ...
            # 상태 파일 확인
            status_file = os.path.join(self.test_dir, 'docs/status.json')
    ...
    def test_concurrent_process_management(self):
        ...
        try:
            ...
            for i in range(process_count):
                ...
                script_path = self.create_load_test_script(f'deploy_latest_report.py', load_type)
...
'stable':_alive_count_> =  len(high_load_processes) * 0.8  # 80% 이상 생존
                }
...
'''.replace('script_name', f'"{script_name}"')
    ...
    def test_immediate_crash_handling(self):
        ...
        try:
            # 즉시 크래시하는 스크립트 생성
            crash_script = self.create_failure_script('test_documentation_standardization.py', 'immediate_crash')
            ...
            # 정상 스크립트로 재시작
            recovery_script_path = os.path.join(self.test_dir, 'posco_news_viewer.py')
...
'''
    ...
    def test_delayed_failure_detection(self):
        ...
        try:
            # 지연 크래시 스크립트 생성
            delayed_crash_script = self.create_failure_script('deploy_latest_report.py', 'delayed_crash')
...
print("/n"_+_" = " * 80)
        ...
        print(f"/n🎯 전체 성공률: {overall_success_rate:.1f}% ({successful_suites}/{total_suites} 스위트)")
...
"""
            ...
            if 'error' in result:
report_+ =  f"- 오류 메시지: {result['error']}/n"
...
print("/n"_+_" = " * 80)
    ...
    if result.wasSuccessful():
        print("/n🎉 모든 엔드투엔드 통합 테스트가 성공했습니다!")
        return 0
    else:
        print("/n❌ 일부 엔드투엔드 통합 테스트가 실패했습니다.")
        return 1
