...
try:
# REMOVED:     from Monitoring/WatchHamster_v3.0/core/performance_monitor.py import (
...
# REMOVED:     from Monitoring/WatchHamster_v3.0/core/performance_optimizer.py import (
        PerformanceOptimizer, OptimizationRecommendation,
        PerformanceIssue, OptimizationCategory, OptimizationPriority
    )
except ImportError as e:
# REMOVED:     print(f"❌ 성능 모니터링 모듈 import 실패: {e}")
    sys.exit(1)

class TestPOSCO News 250808PerformanceMonitoring(unittest.TestCase):
    """성능 모니터링 시스템 테스트"""
        ...
        with patch('psutil.process_iter') as mock_process_iter:
            ...
            mock_processes = [
                Mock(info={
                    'pid': 1234,
                    'name': 'python3',
                    'cmdline': ['python3', '.naming_backup/config_data_backup/watchhamster.log'],
                    'cpu_percent': 15.0,
                    'memory_percent': 25.0
                }),
                Mock(info={
                    'pid': 1235,
                    'name': 'python3',
                    'cmdline': ['python3', 'Monitoring/POSCO_News_250808/posco_main_notifier.py'],
                    'cpu_percent': 10.0,
                    'memory_percent': 20.0
                })
            ]
            ...
            # CPU와 메모리 사용률 모킹
with_patch('psutil.cpu_percent',_return_value = 45.0), /
...
print("/n"_+_" = " * 60)
    ...
    if result.wasSuccessful():
        print("/n🎉 모든 성능 모니터링 테스트가 성공했습니다!")
        return True
    else:
        print("/n⚠️ 일부 테스트가 실패했습니다. 로그를 확인해주세요.")
        return False
