...
duration:_Optional[float] =  None
user:_Optional[str] =  None
system_info:_Optional[Dict[str,_str]] =  None
...
class MigrationStatusReporter:
    ...
    def __init__(self):
        ...
        # 로깅 설정
# REMOVED:         self.log_file = self.log_dir / f"migration_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        self._setup_logging()
...
details:_Dict[str,_Any] =  None, duration: float = None):
        """마이그레이션 이벤트 로깅"""
...
event.phase,_event.message,_json.dumps(event.details,_ensure_ascii = False),
event.duration,_event.user,_json.dumps(event.system_info,_ensure_ascii = False)
            ))
            conn.commit()
...
json.dumps(snapshot.active_processes,_ensure_ascii = False),
json.dumps(snapshot.file_checksums,_ensure_ascii = False)
                ))
                conn.commit()
...
total_duration_+ =  duration
        ...
        # 보고서 파일 저장
        report_file = self.reports_dir / f"file_renaming_simulation_report.txt"
with_open(report_file,_'w',_encoding = 'utf-8') as f:
    ...
    def cleanup_old_logs(self, days: int = 30):
        ...
        # 오래된 로그 파일 삭제
        for log_file in self.log_dir.glob("file_renaming.log"):
...
SUM(CASE_WHEN_event_type =  'success' THEN 1 ELSE 0 END) as success_count,
SUM(CASE_WHEN_event_type =  'error' THEN 1 ELSE 0 END) as error_count,
SUM(CASE_WHEN_event_type =  'warning' THEN 1 ELSE 0 END) as warning_count
                FROM migration_events 
WHERE_session_id =  ?
            ''', (self.session_id,))
