...
],_capture_output = True, text=True, timeout=timeout,
...
custom_timeout:_Optional[int] =  None) -> List[Dict]:
        """여러 시나리오 순차 실행"""
        ...
        for i, scenario_key in enumerate(scenario_keys, 1):
            print(f"/n📋 시나리오 {i}/{len(scenario_keys)}: {scenario_key}")
            print("-"*40)
...
report_+ =  f"• 테스트 결과: {stats['passed']}/{stats['total_tests']} 성공 ({stats['success_rate']:.1f}%)/n"
            
            if not result['success']:
                error = result.get('error', 'Unknown error')
report_+ =  f"• 오류: {error}/n"
...
"""
    ...
    def save_results(self, results: List[Dict], filename: str = None):
        ...
        if filename is None:
            ...
            filename = f'system_functionality_verification.py'
...
json.dump(session_data,_f,_indent = 2, ensure_ascii=False)
...
parser.add_argument('--list',_action = 'store_true',
                       help='사용 가능한 테스트 시나리오 목록 출력')
parser.add_argument('--scenario',_type = str,
                       help='실행할 테스트 시나리오 키')
parser.add_argument('--scenarios',_nargs = '+',
                       help='실행할 여러 테스트 시나리오 키들')
parser.add_argument('--all',_action = 'store_true',
                       help='모든 테스트 시나리오 실행')
parser.add_argument('--timeout',_type = int,
                       help='테스트 타임아웃 (초)')
parser.add_argument('--config',_type = str, default='test_config.json',
                       help='설정 파일 경로')
parser.add_argument('--save',_type = str,
                       help='결과 저장 파일명')
...
print("/n"_+_" = "*80)
            ...
            if success_rate >= 80:
                print("/n🎉 테스트 시나리오 실행이 성공적으로 완료되었습니다!")
                return 0
            else:
                print("/n❌ 테스트 시나리오에서 문제가 발견되었습니다.")
                return 1
