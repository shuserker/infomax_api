        ...
        try:
            ...
            for pattern in important_patterns:
                for file_path in Path(".").glob(f"**/{pattern}"):
                    if not any(exclude in str(file_path) for exclude in [".git", "__pycache__", ".backup"]):
...
class ImportRepairer:
    ...
    def _fix_import_paths(self, content: str) -> Tuple[str, List[str]]:
        ...
        # 일반적인 경로 수정 패턴
        path_fixes = {
            'from Posco_News_mini': 'from POSCO_News_250808',
            'import Posco_News_mini': 'import POSCO_News_250808',
            'from POSCO_WatchHamster_v3': 'from WatchHamster_v3.0',
            'import POSCO_WatchHamster_v3': 'import WatchHamster_v3.0',
        }
