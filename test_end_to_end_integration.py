...
엔드투엔드 통합 테스트 스위트:
- 전체 시스템 시작 및 종료 테스트 시나리오
- 다중 동시 프로세스를 사용한 스트레스 테스트
- 다양한 오류 조건에 대한 실패 시뮬레이션 테스트
...
class MockWatchHamster:
    ...
    def start_managed_processes(self):
        """관리 대상 프로세스 시작 시뮬레이션"""
        processes = ['posco_main_notifier', 'realtime_news_monitor', 'integrated_report_scheduler']
...
'''
    ...
    def test_full_system_startup_shutdown(self):
        ...
        try:
            ...
            self.assertTrue(status['v2_enabled'], "v2가 활성화되지 않음")
...
def main():
    """메인 함수"""
    print("🚀 POSCO WatchHamster v2 엔드투엔드 통합 테스트 시작")
    print("=" * 80)
