#!/usr/bin/env python3
"""
POSCO 워치햄스터 v2.0 마이그레이션 후 검증 스크립트
Post-Migration Verification Script for POSCO WatchHamster v2.0

마이그레이션 완료 후 모든 기능이 올바르게 작동하는지 종합적으로 검증합니다.
"""
...
class PostMigrationVerification:
    ...
    def __init__(self):
        ...
        self.log_file = self.script_dir / "post_migration_verification.log"
        self.results_file = self.script_dir / "verification_results.json"
    ...
    def verify_file_structure(self) -> bool:
        ...
        required_files = [
            ...
            "Monitoring/Posco_News_mini_v2/modules.json",
...
try:
    ...
    # v2 컴포넌트 확인
    components = {{}}
    components['v2_enabled'] = getattr(wh, 'v2_enabled', False)
    ...
    def verify_watchhamster_functionality(self) -> bool:
        ...
        try:
            ...
            if process.poll() is None:
                ...
                self._print_colored(f"  ❌ 워치햄스터 실행 실패", 'RED')
                if stderr:
            ...
            execution_time = time.time() - start_time
            self._print_colored(f"  ❌ 워치햄스터 기능 검증 오류: {str(e)}", 'RED')
            self._log_result(
                'watchhamster_functionality',
                'failed',
                f'검증 오류: {str(e)}',
...
try:
    ...
    def run_comprehensive_verification(self) -> bool:
        """종합 검증 실행"""
        self._print_colored("🚀 POSCO 워치햄스터 v2.0 마이그레이션 후 종합 검증 시작", 'CYAN')
