...
function Start-WatchHamster {
    ...
    if (Test-Path "Monitoring\POSCO News 250808_mini\posco_main_notifier.py") {
        Set-Location "Monitoring\POSCO News 250808_mini"
    ...
    } elseif (Test-Path "Monitoring\POSCO News 250808_mini\posco_main_notifier.py") {
        Set-Location "Monitoring\POSCO News 250808_mini"
        Start-Process -FilePath "python" -ArgumentList "posco_main_notifier.py" -WindowStyle Hidden -RedirectStandardOutput "..\..\posco_monitor.log" -RedirectStandardError "..\..\posco_monitor.log"
        Set-Location $SCRIPT_DIR
        Start-Sleep -Seconds 3
...
function Show-MonitoringStatus {
    ...
    if ($processes) {
        ...
        foreach ($process in $processes) {
            ...
            $scriptName = if ($process.CommandLine -like "*POSCO WatchHamster v3.0" }
...
function Show-NewsStats {
    ...
    if (Test-Path "POSCO News 250808_data.json") {
        $fileInfo = Get-Item "POSCO News 250808_data.json"
        $size = [math]::Round($fileInfo.Length / 1KB, 1)
        $modified = $fileInfo.LastWriteTime.ToString("yyyy-MM-dd HH:mm:ss")
        Write-Host "✅ POSCO News 250808_data.json (${size}KB, 수정: $modified)" -ForegroundColor $GREEN
        
        # 간단한 통계
        try {
            $jsonContent = Get-Content "POSCO News 250808_data.json" | ConvertFrom-Json
        ...
        Write-Host "❌ POSCO News 250808_data.json (없음)" -ForegroundColor $RED
    }
...
function Show-SystemStatus {
    ...
    $requiredFiles = @("Monitoring\POSCO News 250808_mini\posco_main_notifier.py", "Monitoring\POSCO News 250808_mini\monitor_WatchHamster.py", "Monitoring\POSCO News 250808_mini\config.py", "requirements.txt")
    Test-RequiredFiles $requiredFiles
    
    # 데이터 파일 확인
    Show-Section "📊 데이터 파일 상태"
    $dataFiles = @("POSCO News 250808_data.json", "POSCO News 250808_cache.json")
...
function Start-SystemTest {
    ...
    # 파일 시스템 테스트
    $testFiles = @("Monitoring\POSCO News 250808_mini\monitor_WatchHamster.py")
    ...
    } elseif (Test-Path "Monitoring\POSCO News 250808_mini\monitor_WatchHamster.py") {
        try {
            python -c "import sys; print('Python 스크립트 테스트 통과')" 2>$null
            Show-Success "🐹 POSCO WatchHamster v3.0 테스트 통과"
        }
        catch {
            Show-Error "🐹 POSCO WatchHamster v3.0 테스트 실패"
        }
    } else {
...
function Start-DataBackup {
    ...
    # 중요 데이터 파일들 백업
    $dataFiles = @("POSCO News 250808_data.json", "POSCO News 250808_cache.json", "*.py", "config.py")
    $backedUp = $false
