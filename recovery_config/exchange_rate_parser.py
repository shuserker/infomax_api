...
class ExchangeRateParser:
    ...
    def parse_exchange_rate_data(self, raw_data: Dict[str, Any]) -> ExchangeRateData:
        ...
        Args:
            raw_data (dict): 원시 서환마감 데이터
        
        Returns:
            ExchangeRateData: 파싱된 서환마감 데이터
        """
        if not raw_data:
    ...
    def _extract_usd_krw_rate(self, title: str, content: str) -> Optional[CurrencyRate]:
        ...
        Returns:
            Optional[CurrencyRate]: 원달러 환율 데이터
        """
    ...
    def _extract_major_currencies(self, title: str, content: str) -> List[CurrencyRate]:
        ...
        Returns:
            List[CurrencyRate]: 주요 통화 환율 리스트
        """
        currencies = []
        text = f"{title} {content}"
    ...
    def _extract_high_low(self, text: str, position: int) -> Tuple[Optional[float], Optional[float]]:
        ...
        Returns:
            Tuple[Optional[float], Optional[float]]: (고가, 저가)
        """
    ...
    def _determine_market_situation(self, title: str, content: str, 
        ...
        Args:
            title (str): 제목
            content (str): 내용
            usd_krw_rate (Optional[CurrencyRate]): 원달러 환율
    ...
    def _generate_market_summary(self, situation: str, usd_krw_rate: Optional[CurrencyRate], 
        ...
        Args:
            situation (str): 시장 상황
            usd_krw_rate (Optional[CurrencyRate]): 원달러 환율
            factors (List[MarketFactor]): 변동 요인들
...
if __name__ == "__main__":
    ...
    sample_data = {
        ...
        'content': '원달러 환율이 1,352.50원으로 전일 대비 +8.30원(+0.62%) 상승 마감했습니다. 연준 위원들의 매파적 발언이 달러 강세를 이끌었습니다. 엔화는 100엔당 912.45원, 유로는 1,456.78원을 기록했습니다. 거래량은 평소보다 20% 증가한 185억 달러를 기록했습니다. 내일은 미국 고용지표 발표를 앞두고 변동성이 클 것으로 전망됩니다.',
        'date': '20250812',
        'time': '163200'
    }
    ...
    # 데이터 파싱
    parsed_data = parser.parse_exchange_rate_data(sample_data)
    
    print("파싱 결과:")
    print(f"제목: {parsed_data.title}")
    print(f"시장 상황: {parsed_data.market_situation}")
    print(f"변동성: {parsed_data.volatility_level}")
    print(f"시장 요약: {parsed_data.market_summary}")
    print()
    
    if parsed_data.usd_krw_rate:
        rate = parsed_data.usd_krw_rate
        direction = "📈" if rate.direction == "up" else "📉" if rate.direction == "down" else "➡️"
        print(f"원달러 환율:")
        print(f"  {direction} {rate.currency_pair}: {rate.rate:,.2f} ({rate.change:+.2f}, {rate.change_percent:+.2f}%)")
        if rate.high and rate.low:
            print(f"  고가: {rate.high:,.2f}, 저가: {rate.low:,.2f}")
        print()
    
    if parsed_data.major_currencies:
        print("주요 통화:")
        for currency in parsed_data.major_currencies:
            direction = "📈" if currency.direction == "up" else "📉" if currency.direction == "down" else "➡️"
            print(f"  {direction} {currency.currency_pair}: {currency.rate:,.2f} ({currency.change:+.2f})")
        print()
    
    if parsed_data.market_factors:
        print("변동 요인:")
        for factor in parsed_data.market_factors:
            impact_symbol = "🔴" if factor.impact == "negative" else "🟢" if factor.impact == "positive" else "🟡"
            print(f"  {impact_symbol} [{factor.factor_type}] {factor.description[:50]}...")
        print()
    
    if parsed_data.next_day_outlook:
        print(f"다음날 전망: {parsed_data.next_day_outlook}")