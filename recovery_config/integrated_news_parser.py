...
try:
    from recovery_config.news_data_parser import NewsDataParser, NewsItem, NewsStatus
    from recovery_config.newyork_market_parser import NewYorkMarketParser, NewYorkMarketData
    from recovery_config.kospi_close_parser import KospiCloseParser, KospiCloseData
    from recovery_config.exchange_rate_parser import ExchangeRateParser, ExchangeRateData
except ImportError:
...
class IntegratedNewsParser:
    ...
    def _create_integrated_analysis(self, news_items: Dict[str, NewsItem], 
        ...
        Args:
            news_items (dict): 기본 뉴스 아이템들
            specialized_data (dict): 전문 파싱 데이터
        
        Returns:
            IntegratedNewsData: 통합 분석 데이터
        """
        # 전체 상태 판단
    ...
    def _generate_recommendations(self, news_items: Dict[str, NewsItem], 
        ...
        if 'newyork-market-watch' in specialized_data:
            ny_data = specialized_data['newyork-market-watch']
            if ny_data.get('volatility_level') == 'high':
                recommendations.append("뉴욕시장 높은 변동성 - 주의 깊은 모니터링 필요")
        
        if 'exchange-rate' in specialized_data:
            exchange_data = specialized_data['exchange-rate']
            if exchange_data.get('volatility_level') == 'high':
                recommendations.append("환율 높은 변동성 - 리스크 관리 강화 필요")
    ...
    def validate_raw_data(self, raw_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """
        원시 데이터 유효성 검증
        ...
        if not isinstance(raw_data, dict):
            errors.append("원시 데이터가 딕셔너리가 아님")
            return False, errors
        
        if not raw_data:
            errors.append("원시 데이터가 비어있음")
            return False, errors
        
        # 예상되는 뉴스 타입 확인
        expected_types = ['newyork-market-watch', 'kospi-close', 'exchange-rate']
        found_types = [t for t in expected_types if t in raw_data]
        
        if not found_types:
            errors.append("알려진 뉴스 타입이 없음")
            return False, errors
        
        # 각 뉴스 타입별 기본 구조 확인
        for news_type in found_types:
            news_data = raw_data[news_type]
            if not isinstance(news_data, dict):
                errors.append(f"{news_type}: 뉴스 데이터가 딕셔너리가 아님")
                continue
...
if __name__ == "__main__":
    ...
    # 데이터 유효성 검증
    is_valid, validation_errors = parser.validate_raw_data(sample_data)
    print(f"데이터 유효성: {'✅ 통과' if is_valid else '❌ 실패'}")
    ...
    if is_valid:
        ...
        print(f"파싱 결과: {'✅ 성공' if result.success else '❌ 실패'}")
        ...
        if result.data:
            ...
            print()
            print("통합 분석 결과:")
            print(f"  전체 상태: {data.overall_status}")
            print(f"  시장 요약: {data.market_summary}")
            print(f"  뉴스 개수: {data.status_counts['total']}개")
            print(f"  최신 뉴스: {data.status_counts['latest']}개")
            print(f"  지연 뉴스: {data.status_counts['delayed']}개")
            
            if data.delay_analysis['has_delays']:
                print(f"  최대 지연: {data.delay_analysis['max_delay_minutes']}분")
